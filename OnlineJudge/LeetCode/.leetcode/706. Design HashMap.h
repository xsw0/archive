class MyHashMap {
  ...
  static key_type hasher(key_type key) { return key % Prime; }

  static bool key_equal(key_type lhs, key_type rhs) { return lhs == rhs; }
...
public:
  ...
  MyHashMap() { arr.fill({NullFlag, NullFlag}); }
  ...
  void remove(int key) {
    ...
    auto it = find(arr.begin() + hasher(key), f);
    if (key_equal(it->first, NullFlag))
      return;
    auto end =
        find(it, [](value_type n) { return key_equal(n.first, NullFlag); });
