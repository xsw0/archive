    ...
    *two_way = assign(*first);
    ...
    for (auto pivot = std::next(first); pivot != last; ++pivot) {
        if (compare(*pivot, *two_way)) {
            auto dest = std::upper_bound(first_of_small, two_way + size, *pivot,
                                         compare);
            ...
            *std::prev(dest) = assign(*pivot);
            ...
            auto dest = std::upper_bound(std::next(two_way), last_of_big,
                                         *pivot, compare);
...
static void bubble_down(int *first, size_t size, size_t index) {
    if (index * 2 + 1 < size &&
        Sort::compare(first[index], first[index * 2 + 1])) {
        if (index * 2 + 2 < size &&
            Sort::compare(first[index * 2 + 1], first[index * 2 + 2])) {
            ...
            bubble_down(first, size, index * 2 + 1);
               ...
               Sort::compare(first[index], first[index * 2 + 2])) {
        Sort::swap(first[index], first[index * 2 + 2]);
        bubble_down(first, size, index * 2 + 2);
    }
}
