...
void insertion_sort_impl(RandomAccessIterator first, RandomAccessIterator last,
                         Compare comp) {
    using std::swap;
    if (first != last) {
...
template <class Tp> inline void insertion_sort(Tp **first, Tp **last) {
    insertion_sort((size_t *)first, (size_t *)last, std::less<>());
}

template <class Tp>
inline void insertion_sort(std::__wrap_iter<Tp *> first,
