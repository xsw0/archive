...
std::vector<std::vector<int>> GenerateTestCases(size_t size) {
    ...
    static std::uniform_int_distribution<int> INT(
...
template <typename T>
auto testSort(std::function<void(T &)> f, std::vector<T> testCases) {
    size_t count = 0;
...
int sortTest() {
    ...
    auto result = testSort(
        std::function<void(std::vector<int> &)>(
            [&](std::vector<int> &v) { std::sort(v.begin(), v.end(), cmp); }),
        cases);
    std::cout << std::endl;

    std::cout << (result == testSort(std::function<void(std::vector<int> &)>(
                                         [&](std::vector<int> &v) {
                                             quick_sort_tail(v.begin(), v.end(),
                                                             cmp);
                                         }),
                                     cases))
              << std::endl;
