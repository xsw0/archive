...
int AVL_getBf(const struct AVLImpl *impl) { return impl->bf; }
...
static struct AVLImpl *AVLImpl_Construct(struct AVLImpl *parent,
                                         AVL_VALUE_TYPE value) {
...
AVL_VALUE_TYPE AVL_getValue(const struct AVLImpl *impl) { return impl->value; }
...
void AVL_Erase(AVL *avl, const struct AVLImpl *impl) {
    ...
    int LR;
    if (parent) {
        assert(parent->left == non_const_impl ||
               parent->right == non_const_impl);
        LR = parent->left == non_const_impl ? -1 : 1;
        parent_child =
            parent->left == non_const_impl ? &parent->left : &parent->right;
    }
    ...
    if (!non_const_impl->left && !non_const_impl->right) {
        ...
        struct AVLImpl *pivot =
            non_const_impl->left ? non_const_impl->left : non_const_impl->right;
        if (parent) {
            *parent_child = pivot;
            pivot->parent = parent;
            AVL_rebalance(parent, LR, -1);
