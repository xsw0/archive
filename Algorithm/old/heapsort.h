...
void heapsort_impl(Tp *first, Tp *last, Compare comp) {
    priority_queue<Tp, std::vector<Tp>, Compare> pq(comp);
...
void heapsort_impl(RandomAccessIterator first, RandomAccessIterator last,
                   Compare comp) {
...
template <class Tp> inline void heapsort(Tp **first, Tp **last) {
    heapsort((size_t *)first, (size_t *)last, std::less<>());
}

template <class Tp>
inline void heapsort(std::__wrap_iter<Tp *> first,
