...
void quicksort_impl(RandomAccessIterator first, RandomAccessIterator last,
                    Compare comp) {
    using std::swap;
    ...
    if (first != last) {
        ...
        //        swap(*lm1, *(first + std::uniform_int_distribution<int>(0,
        //        std::distance(first, last) - 1)(re)));
...
// instantiation, forcing a reference Compare
...
template <class Tp> inline void quicksort(Tp **first, Tp **last) {
    quicksort((size_t *)first, (size_t *)last, std::less<>());
}

template <class Tp>
inline void quicksort(std::__wrap_iter<Tp *> first,
                      std::__wrap_iter<Tp *> last) {
...
template <class Compare, class RandomAccessIterator>
void quick_sort_tail_impl(RandomAccessIterator first, RandomAccessIterator last,
                          Compare comp) {
    using std::swap;
...
// instantiation, forcing a reference Compare
template <class RandomAccessIterator, class Compare>
inline void quick_sort_tail(RandomAccessIterator first,
...
template <class Tp> inline void quick_sort_tail(Tp **first, Tp **last) {
    quick_sort_tail((size_t *)first, (size_t *)last, std::less<>());
}

template <class Tp>
inline void quick_sort_tail(std::__wrap_iter<Tp *> first,
                            std::__wrap_iter<Tp *> last) {
...
template <class Tp, class Compare>
inline void quick_sort_tail(std::__wrap_iter<Tp *> first,
                            std::__wrap_iter<Tp *> last, Compare comp) {
    using Comp_ref = typename std::add_lvalue_reference<Compare>::type;
